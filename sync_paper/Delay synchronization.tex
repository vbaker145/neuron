...
\color{red}In \parencite{Levy2012} the authors used the same equation form for connectivity and showed that the synaptic connections in the mouse auditory cortex follow this connection probability.
...
Increasing the cross-section of the SCE increases both the total number of neurons as well as the average number of connections per neuron.
We scale the $C$ parameter in equation \ref{eq:connectivity} to maintain the same average number of connections per neuron across the topologies.
We see that, when separated from the increase in connectivity, the topology itself does not change the wave propagation speed (Figure \ref{fig:delay_topology_avgconn}).
\begin{figure}[!htb]
