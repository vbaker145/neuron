...
{\LARGE One-dimensional traveling waves in neural columns}

\ \\
{\bf \large Vincent Baker$^{\displaystyle 1}$}\\
{\bf \large Luis Cruz Cruz$^{\displaystyle 1}$}\\
{$^{\displaystyle 1}$Drexel University, Department of Physics.}\\
%

%\ \\[-2mm]
{\bf Keywords:} Traveling waves, one-dimensional networks, minicolumns, microcolumns
...
Our one-dimensional model is not only useful for studying the basic properties of traveling waves in neural networks, it also provides a simplified representation of possible wave propagation in columnar or minicolumnar networks found in the cortex.
...
These traveling waves have been observed in the cortex of mammalian brains \parencite{muller2018}\parencite{reimer2010}  as well as in vitro \parencite{wu2008}\parencite{huang2004}, and subsequently have been reproduced in silico \parencite{keane2015}\parencite{senk2020}. 
While the function of traveling waves is unknown \parencite{wu2008}\parencite{muller2018}, proposed functions include motor coordination \parencite{sanes1993}, place field coordination in the hippocampus \parencite{lubernov2009}, and spatiotemporal processing in the visual cortex \parencite{wu2008}\parencite{muller2014}. 
...
Local-area traveling waves in the awake cortex have been observed using recent advances in optical imaging techniques using voltage-sensitive dyes \parencite{wu2008}\parencite{shoham1999}.  
...
The vast majority of studies of traveling waves have focused on two-dimensional waves that spread parallel to the surface (pia) of the brain \parencite{reimer2010}\parencite{keane2015}. 
...
Of interest, there are regions of the brain where there are what seem to be essentially one-dimensional structures \parencite{buxhoeveden2002}\parencite{mountcastle1997}\parencite{rockland2004} typically called micro- or minicolumns. 
...
For this, we use distance-dependent time delays for the propagation of a spike from neuron $i$ to neuron $j$ of $\tau_{ij} = \kappa  D(i,j)$, where $\kappa$ ranges from 0 (signals only take one time step to reach their target neuron) to 4. 

The current I in equation (2) includes the sum of all incoming stimuli into neuron $i$ from other neurons $I_i$ and external stimuli $I_{ie}$ applied to the system. 
...
One of them is a uniform background stimulus applied to each neuron $i$ that depends on whether the neuron is excitatory or inhibitory.
...
This stimulus has the effect of creating waves that originate from any point along an SCE and one of the uses is to study interactions between multiple waves in section 3\ref{sub:wave_initiation}.
...
One of the uses of this stimulus is to measure the speed of the traveling wave in section 3\ref{sub:propagation_speed}.
...
The key parameters of our model to be examined are the connection strength $K$, the characteristic connection length $\lambda$, the fraction of excitatory neurons $P_{exc}$ and the delay parameter $\kappa$.
...
\begin{figure}[!htb]
 ...
 \caption{Raster plot of neuron firing events over time. The model parameters are at the fixed point $\Sigma$. Each dot represents the spike-time emission of a neuron, as in \parencite{senk2020}. Traveling waves can be seen as diagonal structures of dense firing activity. }
...
Cortical traveling waves have been observed with propagating speeds from 0.1 to 0.8 meters per second \parencite{sato2012}.
This is consistent with the action potential propagation speeds in the gray matter of the cortex \parencite{muller2018}.
It is thus clear that the dynamics of action potential propagation will influence wave propagation.
...
To determine this spike delay we exposed populations of excitatory and inhibitory neurons to current pulses of different magnitudes.
...
We increased the connection strength $K$ to provide this stronger stimulus, and found that traveling waves that span the SCE are found starting at $K=18$. 
As $K$ increases the neurons fire with less delay, resulting in faster traveling waves (Figure \ref{fig:delay_k}).
We fix $\Sigma_v = \{K=24,\lambda=2.5,P_{exc}=0.8,\kappa=1.0 \}$ for the remainder of the wave speed experiments.
\begin{figure}[!htb]
...
At $\lambda<2.25$ traveling waves were not observed.
\begin{figure}[!htb]
...
This was most noticeable for SCEs with very dense local connectivity with $C=1.0$ in equation \ref{eq:connectivity}.
To quantify this observation we recorded the initiation sites of all traveling waves that were observed in an SCE under uniform background stimulus (Figure \ref{fig:wave_initiation_sites}).
We measure the fraction of wave initiation events (FIE) per unit length of the SCE.
...
We observed that a single traveling wave initiated by a step stimulus can have regions of higher or lower total firing activity which we quantify by measuring the fractional number of firing events (FFE) per unit length of the SCE.
...
In our experiments we observe an anti-correlation between the FIE and the FFE for an SCE.
The FIE and FFE of the same SCE are shown in figure \ref{fig:initiation_density}.
This visually indicates that the preferential wave initiation sites tend towards lower firing density in the single wave experiments.
\begin{figure}[!htb]
 \caption{FIE measured using background stimulus (left) and FFE measured using step stimulus (right) for the same SCE.}
 \label{fig:initiation_density}
 \centering
   \includegraphics[width=\textwidth]{fig/InitiationCorrelationHistogram}
\end{figure}

To test that this observation is generally true we create 100 SCEs, applied first uniform background stimulus and then step stimulus to each SCE, and measured the FIE and FFE.
Although there is substantial variation between SCEs we measure a consistently negative correlation coefficient (mean $-0.32$, standard deviation $0.16$).
This demonstrates a consistent, negative correlation between FIE and FFE.
...
In fact, postinhibitory rebound spiking is observed in both cortical neurons \parencite{ascoli2010} and the Izhikevich model of neuron dynamics \parencite{izhikevich}.
...
We first created an SCE with entirely excitatory neurons.
We then created an identical SCE except for a single LTS inhibitory neuron at $Z=50$.
The $b$ parameter of the LTS inhibitory neuron was set to $0.25$, corresponding to the lowest firing threshold used in our model.
These two SCEs were stimulated with a uniform background stimulus. 
...
We show that this is possible provided we consider the action potential propagation delay proportional to the distance between neurons.
...
This is analogous to the original Izhikevich simulation \parencite{izzy_code} that demonstrated synchronized firing in a completely connected neural field with random background stimulus.
We first simulated the SCE with a fixed action potential propagation delay of $1~ms$ regardless of inter-neuron distance.
The result was highly synchronized simultaneous firing among all neurons in the SCE.
...
The present case is similar to previous work (\parencite{ermentrout2001}, Figure 1) that classifies several architectures for apparent traveling waves in the cortex.
...
This is a fundamentally different type of traveling wave than a chain of spikes that spreads to neighboring neurons due to local connectivity.
...
The regions identified through a linear stability analysis were sharply defined.
The previous research used highly structured spatial arrangements of excitatory and inhibitory neurons with deterministic connectivity.
Our model randomly assigns neurons as excitatory or inhibitory and uses a probabilistic connection model.
...
This may explain why traveling waves do not dominate the firing activity at a sharply defined parameter region.

Other research \parencite{keane2015} has shown that traveling waves emerge from networks of leaky-integrate-and-fire neurons when the excitation and inhibition are balanced.
In our model traveling waves are supported even in networks consisting entirely of excitatory neurons (Figure \ref{fig:wave_parameters}C).
This may be explained by our use of a two-dimensional model of neuron dynamics.
TODO: EXPLAIN
