...
One-dimensional waves have also been observed in simulations using firing rate neurons \parencite{Roxin2005}, integrate-and-fire neurons \parencite{Bressloff1997}\parencite{Golomb1999} and \color{red}Hodgkin\color{black}-Huxley neurons \parencite{Golomb1997}.
\color{red}
...
\color{red}
We find that a purely one-dimensional version of our model does not support traveling waves, in contrast with previous, more homogenous models. 
\color{black}
...
\begin{figure}[!htb]
 \caption{Example SCE with dimensions 2x2x10 (\color{red}XxYxZ\color{black}), $\lambda$=2.5, and C=1. A)  SCE showing connections between neurons as lines colored using a color scale that indicates the connection length. 
...
\color{red}
We visualize the spikes in spike raster plots (see Figure \ref{fig:wave_analysis} and Figure \ref{fig:sigma_raster} for examples).
Our interest is in traveling waves in the Z direction, so we plot the spikes according to the Z position of the neuron.
There are multiple neurons at each Z position that will contribute to the spike raster plot at that coordinate.
\color{black}
...
To consider the possibility of this relationship we incorporate in our model an action potential propagation time proportional to the distance between the neurons\color{red}.\color{black}
...
To test these, we construct columns with topology in \color{red}XxY\color{black} of $2x2, 2x3, 3x3, 3x4$ and $4x4$.
We also tested the $1x1$ topology but found that it did not support traveling waves (data not shown).
...
\begin{figure}[!htb]
 \caption{ The pace (left, error bars $\pm 1 \sigma$) decreases, and the speed (right) increases, as the X\color{red}x\color{black}Y extents of the SCE grow.}
...
This is generally slower than other computational results, but of the same order of magnitude as the experimental velocity of \color{red}$50~mm/s$ \color{black} reported in \parencite{Golomb1997}.
