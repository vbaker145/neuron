...
Our \color{red}quasi \color{black} one-dimensional model is not only useful for studying the basic properties of traveling waves in neural networks, but it also provides a simplified representation of possible wave propagation in columnar or minicolumnar networks found in the cortex.
...
\color{red}Traveling waves have been detected in local-field potentials measured with electrodes \cite{Rubino2006}\parencite{sanes1993}\parencite{Riehle2013}\color{black}.
...
Studies of traveling waves have focused on two-dimensional waves that spread parallel to the surface (pia) of the brain \parencite{reimer2010}\parencite{keane2015}\color{red}\cite{Townsend2018}\parencite{Golomb1997}\parencite{Qi2015}\color{black}. 
...
\color{red}
Traveling waves in one-dimensional neural systems have been previously explored using several theoretical methods.
...
See for example \parencite{Senk2020} where traveling waves are only observed for certain parameter values, or \parencite{Strogatz1991} where the stability of systems of coupled oscillators can be radically altered by including infitessimal noise.
Our current work explores traveling waves in quasi-one-dimensional networks with substantial randomness in both individual neurons (randomly selecting type and randomizing some dynamical parameters), connectivity between neurons, and stimulus to the neurons.
\color{black}

\color{red}Although these previous studies largely used one-dimensional structures to simplify computational or analytic aproaches, there is no a priori reason that quasi one-dimensional traveling waves may not be found in vivo\color{black}. 
...
\section{Methods}
To study traveling waves in small columnar ensembles (SCE) of neurons \color{red}we created a MATlAB simulation of our model \color{black}. 
...
\color{red}
Each neuron has an average of $6.90$ input connections, as measured across $100$ randomly-generated SCE (see SI Figure 2 for connectivity distribution).
\color{black}
...
In \parencite{Chervin1988} they authors found even slower propagation of $0.06-0.09 m/s$ in disinhibited slices of rat cortex.
...
This indicates that a second key element, namely neuron and synapse dynamics, will also influence wave propagation speed\color{black}.
...
This indicates that the lower firing thresholds that result from increasing the $b$ parameter result in faster traveling waves.
\color{black}
\begin{figure}[!htb]
...
We see that increasing the X and/or Y dimensions of the SCE increases the speed, and the dependence looks qualitateively similar to increasing the connection strength via $K$ or the number of connections via $\lambda$.
...
To separate the effect of the increased connectivity from the effect due to the increased number of neurons we scale the $C$ parameter in equation \ref{eq:connectivity} to maintain the same average number of connections per neuron (~7) across the topologies.
...
By assuming that our unit grid is about $20~\mu m$ \parencite{cruz2005} then our wave propagation speeds are in the range of $2-16~\mu m/ms$ or $2-16~mm/s$.
...
By combining these two experiments we observe that the results from the FIE and the FFE appeared anti-correlated for the same SCE (see Figure 3 in Supplemental Information for examples).
...
\color{reed} In contrast to previous work on purely one-dimensional neuronal traveling waves, we did not observe traveling waves in the purely one-dimensional ($X=1,Y=1$) version of our model. \color{black}
Various parameters determine traveling wave formation and propagation including the connectivity, connection strength \color{red}and \color{black} excitatory/inhibitory balance.
...
\color{red}
The speed of these waves are influenced by both connectivity and neuron dynamics, with a logarithmic dependence on connection strength that agrees with previous work.
\color{black}
...
\color{red}We have shown that the action potential propagation speed has a linear effect on the eave propagation speed but does not effect the existence of traveling waves\color{black}.
...
Although the stability analysis shows sharply defined regions (their Figure 3 and 4), their spiking neuron simulation results show a gradual transition between regions (their Figure 7) in agreement with our work.

We have explored the quasi one-dimensional nature of our system by examining SCE with larger X and Y extents (although still much less than the Z extent).
We found that the speed of the traveling waves initiated at the bottom of the SCE increased due to the higher connectivity between the neurons, but the speed was constant when the connectivity was regularized for SCE thickness.
We also observed (SI Figure 5) that, when all neurons were stimulated, there seems to be a phase transition from traveling waves to synchronous firing as the SCE thickness increases.
\color{black}
...
\color{red}
Our connection probability model also allows for a small number of very long connections, but these do not seem to be significant (see SI Figure 4).
\color{black}
