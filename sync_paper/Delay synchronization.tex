...
To test that this observation is generally true we create 100 SCEs, applied first uniform background stimulus and then step stimulus to each SCE, and measured the correlation between the FIE and FFE (Figure \ref{fig:InitiationCorrelation}).
Although there is substantial variation between SCEs we measure a consistently negative correlation coefficient (mean $-0.26$, standard deviation $0.18$).
...
These two SCEs were stimulated with the same uniform background stimulus. 
...
When excitation dominates then only plane waves were observed.
