...
To determine whether the preferential firing sites were established by inhibitory neurons with a higher $b$ parameter we simulated a column with only one inhibitory neuron.
The single inhibitory neuron had the maximum value of $b=0.25$.
Figure \ref{fig:initiation_pacemaker} demonstrates that the single inhibitory neuron establishes the traveling waves in the column.
\begin{figure}[!htb]
 \caption{The single low-threshold inhibitory neuron at $Z=20$ is responsible for generating the traveling waves in this experiment. }
 \label{fig:initiation_pacemaker}
 \centering
   \includegraphics[width=\textwidth]{fig/PacemakerNeuron}
\end{figure}
...
Our model shows preferential sites for wave initiation due to the presence of inhibitory neurons with low firing thresholds.
