...
The other model parameters were first fixed at $\Sigma = \{K=10,\lambda=2.5,P_{exc}=0.8,\kappa=1.0 \}$ as in the preceding section.
...
Traveling waves spawned by uniform background stimulation do not appear to originate at random places in the column.
Instead we observe that, for a given randomly generated column, waves tend to spawn from certain locations within the column.
This was most noticeable for columns with very dense local connectivity with $C=1.0$ in equation \ref{eq:connectivity}.
To quantify this observation we recorded the initiation sites of all traveling waves that were observed in a column under uniform background stimulus (Figure \ref{fig:wave_initiation_sites}).

\begin{figure}[!htb]
 \caption{Left: The time and place of origin of every traveling wave produced during a simulation. Right: The histogram of wave initiation sites clearly shows that traveling waves are likely to start in some regions, and unlikely to start in others.}
...
An example measurement of the firing density in a single traveling wave is shown in Figure \ref{fig:wave_density}.
\begin{figure}[!htb]
 \caption{Density of firing events in a traveling wave initiated from a step stimulus. Left: Raster plot of the spike-emission times from a single traveling wave. Right: a histogram of firing events by position shows the density of the wave.}
 \label{fig:wave_density}
 \centering
   \includegraphics[width=\textwidth]{fig/ImpulseWaveDensity}
\end{figure}

In our experiments we observe an anti-correlation between the preferential wave initiation sites and the wave density for a column.
The wave initiation sites and wave density of the same column are shown in figure \ref{fig:initiation_density}.
This visually indicates that the preferential firing sites tend towards lower firing density in the single wave experiments.
\begin{figure}[!htb]
...
To test this hypothesis we create 100 columns, applied first uniform background stimulus and then step stimulus to each column, and measured the correlation between wave initiation and firing density at the positions of the column.
Although there is substantial variation between columns we measure a consistently negative correlation coefficient (mean $-0.32$, standard deviation $0.16$).
This demonstrates a consistent, negative correlation between wave initiation sites and wave density.
...
This indicates the preferential wave initiation sites could be due to low-threshold spiking (LTS) inhibitory neurons \cite{izhikevich2003}.
...
As a traveling wave passes through the region, these same inhibitory neurons suppress the surrounding neurons resulting in fewer local firing events.

A comparative experiment shows that low-threshold spiking inhibitory neurons can indeed generate traveling waves under background stimulus.
...
These two columns were stimulated with a uniform background stimulus. 
