...
\begin{figure}[!htb]
 ...
 \color{red}The labels of the neurons used in both axes are sequentially assigned starting at the bottom (Z=0).
 C) The sum of presynaptic weights for each neuron shows the anisotropy of this model, with substantial variation in input strength and sign between the neuron inputs.\color{black}}
...
\begin{figure}[!htb]
 ...
 \caption{Wave identification and labeling \color{red}using an example SCE with dimensions 2x2x200 \color{black}. Left: Raster plot of firing events where dots represent neuronal action potentials. 
...
\color{red}As we can see from figure \ref{fig:delay_neurondynamics} \color{black} the $b$ parameter in the Izhikevich model sets the firing threshold of the individual neurons.
...
The present case is similar to previous work \color{red}that considered coupled oscillators \color{black} (\parencite{ermentrout2001}, Figure 1) that proposed several models for traveling waves in the cortex.
...
\color{black}
\color{red} We have also shown that \color{black} the connectivity and connection strengths influence the wave propagation speed \color{red}(Figures \ref{fig:delay_k} and \ref{fig:delay_topology}) \color{black} through the neuron dynamics \color{red}(Figure \ref{fig:delay_neurondynamics}) \color{black}.
...
We have found that low-threshold-spiking inhibitory neurons can determine the origin point of traveling waves when using a uniform background stimulus \color{red}(Figure \ref{fig:lts_inhibit})\color{black}.
\color{red}
