...
para.tmin=0; para.tmax=100; para.dts=0.000000000000001;
...
for bc=bc_vect
    ...
    if num_plots>0
        for mc=1:length(measures)
            ...
            if mod(plotting,4)>1
                ...
                if ismember(measure,[2 10])                                             % Group profiles for SPIKE-Sync need extra treatment since support is different for each group
    ...
    bm_results(bc,1:2)=[SPIKY_loop_results.ISI.overall SPIKY_loop_results.SPIKE.overall];
end
