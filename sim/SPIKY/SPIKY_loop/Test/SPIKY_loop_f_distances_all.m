...
function results=SPIKY_loop_f_distances_all(spikes,d_para)

m_para.all_measures_str={'PSTH';'C';'I';'S';'S_r';'S_f';'S_N';'S_E';'S_{NE}';'Sd';'Sd_N';'Sd_E';'Sd_{NE}'}; % 1:SPIKE-Sync,2:ISI,3:SPIKE,4:realtimeSPIKE,5:forwardSPIKE,6:PSTH
m_para.all_measures_string={'PSTH';'SPIKE_Synchro';'ISI';'SPIKE';'SPIKE_realtime';'SPIKE_forward';'SPIKE_N';'SPIKE_E';'SPIKE_NE';'SPIKE_disc';'SPIKE_disc_N';'SPIKE_disc_E';'SPIKE_disc_NE'};
...
m_para.psth=1;
m_para.spikesync_disc=2;
m_para.isi_pico=3;
m_para.spike_pili=4;
m_para.realtime_spike_pili=5;
m_para.forward_spike_pili=6;
m_para.spike_neb_pili=7;
m_para.spike_eero_pili=8;
m_para.spike_neb_eero_pili=9;
m_para.spike_disc=10;
m_para.spike_neb_disc=11;
m_para.spike_eero_disc=12;
m_para.spike_neb_eero_disc=13;
...
m_para.disc_measures=[m_para.spikesync_disc m_para.spike_disc m_para.spike_neb_disc m_para.spike_eero_disc m_para.spike_neb_eero_disc];
...
aaa=f_para.select_measures(m_para.num_non_bi_measures+1:m_para.num_all_measures);
...
%             run_firsts=zeros(1,d_para.num_trains);
%             run_lasts=zeros(1,d_para.num_trains);
%             for trac=1:d_para.num_trains
%                 run_firsts(trac)=find(uspikes{trac}(1:num_uspikes(trac))<=m_res.cum_isi(r_para.run_pico_starts(ruc)),1,'last');
%                 run_lasts(trac)=find(uspikes{trac}(1:num_uspikes(trac))<m_res.cum_isi(r_para.run_pico_ends(ruc)+1),1,'last');
%                 if run_lasts(trac)==num_uspikes(trac)
...
%                     run_lasts(trac)=num_uspikes(trac)-1;
%                 end
%                 run_ivs{trac}=ivs{trac}(run_firsts(trac):run_lasts(trac))-r_para.run_pico_starts(ruc)+1;
%                 run_ive{trac}=ive{trac}(run_firsts(trac):run_lasts(trac))-r_para.run_pico_starts(ruc)+1;
%                 run_ivs{trac}(run_ivs{trac}<1)=1;
...
%             run_num_ints=run_lasts-run_firsts+1;
        ...
        if m_para.num_pili_measures>0
            ...
            if any(f_para.select_measures(m_para.spike_measures))                                                   % any SPIKE-measure
                ...
                if any(f_para.select_measures([m_para.spike_eero_pili m_para.spike_neb_eero_pili m_para.spike_eero_disc m_para.spike_neb_eero_disc]))      % Eero-correction
                    ints2=ints;
                    if f_para.isi_thr>=0
                        if f_para.isi_thr==0
                ...
                if any(f_para.select_measures([m_para.spike_neb_pili m_para.spike_neb_disc]))                                                   % SPIKE-Neb-pili
                ...
                if any(f_para.select_measures([m_para.spike_eero_pili m_para.spike_eero_disc]))                                                   % SPIKE-Eero-pili
                ...
                if any(f_para.select_measures([m_para.spike_neb_eero_pili m_para.spike_neb_eero_disc]))                                                   % SPIKE-Neb-Eero-pili
        ...
        if m_para.num_disc_measures>0                                                                                                   % disc
                        ...
                        for trac2=trac1+1:d_para.num_trains
                            ...
                            if num_pair_all_spikes(pac)>0
                                ...
                                if r_para.num_runs==1
                                    [pac trac1 trac2]
                                    spike_synchro{pac}
                                    sum(any(run_all_pos_spikes))
                                    norm_spike_synchro(pac,any(run_all_pos_spikes))=spike_synchro{pac};
                                else
                                    if ruc==1
                                        norm_spike_synchro(pac,any(run_all_pos_spikes))=spike_synchro{pac}(1:sum(any(run_all_pos_spikes)));
                                    else
                                        start=sum(any(m_res.all_pos_spikes([trac1 trac2],1:r_para.run_disc_starts(ruc)-1)));
                                        norm_spike_synchro(pac,any(run_all_pos_spikes))=spike_synchro{pac}(start+(1:sum(any(run_all_pos_spikes))));
                                    end
                                end
                            end
                        end
            ...
            spike_disc_vect=[m_para.spike_disc m_para.spike_neb_disc m_para.spike_eero_disc m_para.spike_neb_eero_disc];
            if any(f_para.select_measures(spike_disc_vect))
                spike_pili_vect=[m_para.spike_pili m_para.spike_neb_pili m_para.spike_eero_pili m_para.spike_neb_eero_pili];
                sel_spike=find(f_para.select_measures(spike_disc_vect));
        ...
        if r_para.num_runs>1
            ...
            if m_para.num_disc_measures>0
                eval(['disc_' num2str(ruc) '= m_res.disc_measures_mat;']);
            ...
            if m_para.num_pico_measures>0
                eval(['pico_' num2str(ruc) '= m_res.pico_measures_mat;']);
                if ruc==r_para.num_runs
            ...
            if m_para.num_pili_measures>0
                eval(['pili_' num2str(ruc) '= m_res.pili_measures_mat;']);
                if ruc==r_para.num_runs
            ...
            save(['SPIKY_AZBYCX' num2str(ruc)],'-v6',['pi*' num2str(ruc)])
...
if m_para.memo_num_measures>0                                               % ##### pico & pili #####
    if m_para.num_disc_measures>0
        ...
        if last_winspike_ind2<length(sel_all_cum_isi2) && f_para.tmax>sel_all_cum_isi2(last_winspike_ind2)
            ...
            last_winspike_ind2=last_winspike_ind2+1;   % pico: num_isi instead of num_spikes
        end
        win_sel_spikes=sel_spikes(first_winspike_ind2:last_winspike_ind2);
        isi2=diff(win_cum_isi2);
        num_spikes=length(isi2)+1;
    end
...
if m_para.num_sel_measures>0
    ...
    if m_para.memo_num_measures>0                                               % ##### pico & pili #####
        ...
        if r_para.num_runs>1 || num_profiles>0
            if m_para.num_disc_measures>0
                num_event_profiles=mod(f_para.profile_mode,2)+2*sum(f_para.num_select_group_trains>1)*(f_para.profile_mode>1);
                if f_para.select_measures(m_para.spikesync_disc)
                    spike_sync=zeros(num_event_profiles,num_spikes);
                end
            end
            if m_para.num_pico_measures>0
                ...
                if m_para.num_disc_measures>0
                    if f_para.select_measures(m_para.spikesync_disc)
                        if first_winspike_ind2==last_winspike_ind2-1
                            ...
                            spike_sync(1,1:num_spikes)=double(sum(shiftdim(m_res.disc_measures_mat(m_para.measure_indy(m_para.spikesync_disc),...
    ...
    if m_para.num_disc_measures>0
        if f_para.select_measures(m_para.spikesync_disc)
            time=[0 cumsum(m_res.all_isi)];
            if length(spike_sync)>2
                if event_num==1
                    event_indy=2:length(spike_sync)-1;
                elseif event_num==2
                    event_indy=1:length(spike_sync)-1;
                elseif event_num==3
                    event_indy=2:length(spike_sync);
                elseif event_num==4
                    event_indy=1:length(spike_sync);
                end
                spike_sync=spike_sync(:,event_indy);
                time=time(event_indy);
            end
            eval(['results.',char(m_para.all_measures_string(m_para.spikesync_disc)),'.time=time;'])
            eval(['results.',char(m_para.all_measures_string(m_para.spikesync_disc)),'.profile=spike_sync;'])
        end
    end
    if m_para.num_pico_measures>0
        for mc=1:m_para.num_pico_measures
            eval(['results.',char(m_para.all_measures_string(m_para.select_pico_measures(mc))),'.time=m_res.isi;'])
        end
        if f_para.select_measures(m_para.isi_pico)
            eval(['results.',char(m_para.all_measures_string(m_para.isi_pico)),'.profile=isi_ratio;'])
        end
    end
    if m_para.num_pili_measures>0
        for mc=1:m_para.num_pili_measures
            eval(['results.',char(m_para.all_measures_string(m_para.select_pili_measures(mc))),'.time=m_res.pili_supi;'])
        end
        if f_para.select_measures(m_para.spike_pili)
            eval(['results.',char(m_para.all_measures_string(m_para.spike_pili)),'.profile=spike_diffs_l;'])
        end
        if f_para.select_measures(m_para.realtime_spike_pili)
            eval(['results.',char(m_para.all_measures_string(m_para.realtime_spike_pili)),'.profile=spike_diffs_realtime_l;'])
        end
        if f_para.select_measures(m_para.forward_spike_pili)
            eval(['results.',char(m_para.all_measures_string(m_para.forward_spike_pili)),'.profile=spike_diffs_forward_l;'])
        end
    end
    ...
    if f_para.select_measures(m_para.psth) && num_profiles>0
        ...
        results.PSTH.x=bins(1:f_para.psth_num_bins)+(bins(2)-bins(1))/2;
