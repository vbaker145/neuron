...

{
    ...
    double *bi_spike_diffs, *udists, *udists2, *isi_pos, *foll_spikes, *prev_spikes, *ints;
    ...
    isi_pos = (double *)mxGetPr(isi_pos_in);
    ints = (double *)mxGetPr(ints_in);
    foll_spikes_indy = (int *)mxGetPr(foll_spikes_indy_in);
    prev_spikes = (double *)mxGetPr(prev_spikes_in);
    
    bi_spike_diffs_out = mxCreateNumericArray(0, 0, mxDOUBLE_CLASS, mxREAL);
    mxSetM(bi_spike_diffs_out, *num_pairs);
    mxSetN(bi_spike_diffs_out, *run_pico_lengths_ruc);
    mxSetData(bi_spike_diffs_out, mxMalloc(sizeof(double) * *num_pairs * *run_pico_lengths_ruc));
    bi_spike_diffs = (double *)mxGetPr(bi_spike_diffs_out);
