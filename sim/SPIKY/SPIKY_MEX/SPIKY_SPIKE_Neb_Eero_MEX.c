...

{
    ...
    double *bi_spike_diffs_t, *udists, *udists2, *folli, *previ, *ints, *ints2;
    ...
    for(trac1 = 0; trac1 < *num_trains-1; ++trac1)
        for(trac2 = trac1 + 1; trac2 < *num_trains;  ++trac2) {
            ...
            for(sac = 0; sac < *run_time_lengths_ruc; ++sac)
                ...
                + udists[foll_indy[trac1 + *num_trains*sac] - 1]*previ[trac1 + *num_trains*sac])/ints[trac1 + *num_trains*isi_indy[sac]]
                + (udists2[prev_indy[trac2 + *num_trains*sac] - 1]*folli[trac2 + *num_trains*sac]
                +  udists2[foll_indy[trac2 + *num_trains*sac] - 1]*previ[trac2 + *num_trains*sac])/ints[trac2 + *num_trains*isi_indy[sac]])
                /((ints2[trac1 + *num_trains*isi_indy[sac]] + ints2[trac2 + *num_trains*isi_indy[sac]])/2);
//                 bi_spike_diffs_t[(pac-1) + *num_pairs * sac] = ((udists[prev_indy[trac1 + *num_trains * sac] - 1]*folli[trac1 + *num_trains * sac]
