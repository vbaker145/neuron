...
#define spikes_in prhs[0]
    ...
    unsigned dummy;
    int num_trains, *num_sac, trac1, trac2, num_spikes1, num_spikes2, usac, changeSign, cntr1, cntr2, x, totalCounter, temp, temp2, flag[2][2], indx[2], flag1 = 1;
    ...
    for(trac1 = 0; trac1 < num_trains - 1; ++trac1) {
        ...
        for(dummy = 0; dummy < num_spikes1; ++dummy) //
        ...
        for(trac2 = trac1+1; trac2 < num_trains;  ++trac2) {
            ...
            mexPrintf("\tTheir values are \n"); //
            for(dummy = 0; dummy < num_spikes2; ++dummy) //
            ...
            if (num_spikes1 == 0) {
                ...
                if (num_spikes2 == 0) {
                    ...
                    //for(unsigned cntrii = 0; cntrii < 20; ++cntrii) {//
                    flag1 = 1;
                    
                    while (((indx[0] < num_spikes1 - 1) || (indx[1] < num_spikes2 - 1)) || flag1) {
                        
                        if ((indx[0] == num_spikes1-1) && (indx[1] == num_spikes2-1)) {
                               flag1 = 0;
                               mexPrintf(" flag1 = 0!!!!!!!!!!!!!!!!!!!1\n");
                        }
                        ...
                        if (indx[0] < num_spikes1  && indx[1] < num_spikes2) {
                            ...
                            mexPrintf("\t\t\t spike1 = %f, spikes2 = %f \n", spikes1[indx[0]], spikes2[indx[1]]);
            ...
            for(dummy = 0; dummy < num_spikes1 + num_spikes2; ++dummy) //
