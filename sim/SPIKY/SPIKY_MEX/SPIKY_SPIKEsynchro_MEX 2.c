...
#define uspikes_in prhs[2]
    ...
    bool *synchro, *synchroReallocated;
    ...
    for(trac1 = 0; trac1 < *num_trains; ++trac1)
        for(trac2 = trac1+1; trac2 < *num_trains;  ++trac2) {
            ...
            synchroPr = mxCreateNumericArray(0, 0, mxLOGICAL_CLASS, mxREAL);
            mxSetM(synchroPr, 1);
            if (num_uspikes[trac1] + num_uspikes[trac2] == 0) {
                mxSetN(synchroPr, 1);
                mxSetData(synchroPr, mxMalloc(sizeof(bool)));
            }
            else {
                mxSetN(synchroPr, num_uspikes[trac1] + num_uspikes[trac2]);
                mxSetData(synchroPr, mxMalloc(sizeof(bool) * (num_uspikes[trac1] + num_uspikes[trac2])));
            }
            synchro = (bool *)mxGetPr(synchroPr);
